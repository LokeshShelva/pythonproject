...
def Bar_API(graph_data):
    ...
    df['label'] = name
    df['color'] = color
    graph_data['yAxis']['minRange'],graph_data['yAxis']['maxRange'] = float(graph_data['yAxis']['minRange']),float(graph_data['yAxis']['maxRange'])
...
def Grouped_Bar_API(graph_data):
    ...
    for f in dataSet[0]['field']:
        df[f] = []
    for entry in dataSet:
        row_val = [entry['name']]+[int(x) for x in entry['value']]
...
def Scatter_API(graph_data):
    ds = graph_data['data']
    for entry in ds:
        x = [int(i[1]) for i in entry['value']]
        y = [int(i[0]) for i in entry['value']]
        new = {'x':x,'y':y}
        entry.update(new)
    dictionary = {
...
API_type_dictionary = {'B':Bar_API,'C':Grouped_Bar_API,'S':Scatter_API,'L':Line_API,'P':Pie_API}
